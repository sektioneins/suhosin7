...
{ /* blocksize=32*nb bits. Key=32*nk bits */
  ...
  /* pre-calculate forward and reverse increments */
    ...
    {
        SUHOSIN7_G(fi)[m]=(j+C1)%nb;
        SUHOSIN7_G(fi)[m+1]=(j+C2)%nb;
        SUHOSIN7_G(fi)[m+2]=(j+C3)%nb;
        SUHOSIN7_G(ri)[m]=(nb+j-C1)%nb;
        SUHOSIN7_G(ri)[m+1]=(nb+j-C2)%nb;
        SUHOSIN7_G(ri)[m+2]=(nb+j-C3)%nb;
    }
    ...
    for (i=0;i<Nk;i++) SUHOSIN7_G(fkey)[i]=CipherKey[i];
    for (j=Nk,k=0;j<N;j+=Nk,k++)
    {
        SUHOSIN7_G(fkey)[j]=SUHOSIN7_G(fkey)[j-Nk]^SubByte(ROTL24(SUHOSIN7_G(fkey)[j-1]))^rco[k];
        if (Nk<=6)
        {
            for (i=1;i<Nk && (i+j)<N;i++)
                SUHOSIN7_G(fkey)[i+j]=SUHOSIN7_G(fkey)[i+j-Nk]^SUHOSIN7_G(fkey)[i+j-1];
        }
        else
        {
            for (i=1;i<4 &&(i+j)<N;i++)
                SUHOSIN7_G(fkey)[i+j]=SUHOSIN7_G(fkey)[i+j-Nk]^SUHOSIN7_G(fkey)[i+j-1];
            if ((j+4)<N) SUHOSIN7_G(fkey)[j+4]=SUHOSIN7_G(fkey)[j+4-Nk]^SubByte(SUHOSIN7_G(fkey)[j+3]);
 ...
 /* now for the expanded decrypt key in reverse order */
    ...
    {
        k=N-Nb-i;
        for (j=0;j<Nb;j++) SUHOSIN7_G(rkey)[k+j]=InvMixCol(SUHOSIN7_G(fkey)[i+j]);
    }
    for (j=N-Nb;j<N;j++) SUHOSIN7_G(rkey)[j-N+Nb]=SUHOSIN7_G(fkey)[j];
}
   ...
   loop and hard-code in the values of fi[]  */
        ...
        { /* deal with each 32-bit element of the State */
          /* This is the time-critical bit */
            y[j]=SUHOSIN7_G(fkey)[k++]^ftable[(BYTE)x[j]]^
                 ROTL8(ftable[(BYTE)(x[SUHOSIN7_G(fi)[m]]>>8)])^
                 ROTL16(ftable[(BYTE)(x[SUHOSIN7_G(fi)[m+1]]>>16)])^
                 ROTL24(ftable[x[SUHOSIN7_G(fi)[m+2]]>>24]);
        }
...
/* if Nb is fixed - unroll this next
   loop and hard-code in the values of ri[]  */
        ...
        for (m=j=0;j<Nb;j++,m+=3)
        { /* This is the time-critical bit */
            y[j]=SUHOSIN7_G(rkey)[k++]^rtable[(BYTE)x[j]]^
                 ROTL8(rtable[(BYTE)(x[SUHOSIN7_G(ri)[m]]>>8)])^
                 ROTL16(rtable[(BYTE)(x[SUHOSIN7_G(ri)[m+1]]>>16)])^
                 ROTL24(rtable[x[SUHOSIN7_G(ri)[m+2]]>>24]);
        }
        t=x; x=y; y=t;      /* swap pointers */
    }
