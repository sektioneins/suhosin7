...
static int check_fileupload_varname(char *varname)
{
	...
	if (SUHOSIN7_G(max_post_totalname_length) && SUHOSIN7_G(max_post_totalname_length) < var_len) {
...
int suhosin_rfc1867_filter(unsigned int event, void *event_data, void **extra)
{
	...
	switch (event) {
		...
		case MULTIPART_EVENT_FILE_START:
			{
				...
				if (SUHOSIN7_G(upload_limit) && SUHOSIN7_G(upload_limit) <= SUHOSIN7_G(num_uploads)) {
					suhosin_log(S_FILES, "configured fileupload limit exceeded - file dropped");
					if (!SUHOSIN7_G(simulation)) {
						SUHOSIN7_G(no_more_uploads) = 1;
						goto continue_with_failure;
					}
				}
		...
		case MULTIPART_EVENT_FILE_DATA:
			...
			if (SUHOSIN7_G(upload_disallow_elf)) {
...
#ifdef SUHOSIN7_EXPERIMENTAL
					if ((*cp & 0x80) && SUHOSIN7_G(upload_allow_utf8)) {
...
#ifdef SUHOSIN7_EXPERIMENTAL
					else if (SUHOSIN7_G(upload_allow_utf8) && mefd->data[i] & 0x80) {
